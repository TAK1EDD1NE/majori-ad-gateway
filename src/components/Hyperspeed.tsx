...
/* Reference: Hyperspeed (ReactBits) — ported as provided by the user.
   Only TypeScript strictness fixes (noUncheckedIndexedAccess assertions). */
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS, lightMode = false }) => {
  ...
  useEffect(() => {
    ...
    if (typeof options.distortion === 'string') {
      options.distortion = (distortions[options.distortion] ?? defaultOptions.distortion) as Exclude<
        typeof options.distortion,
        string | undefined
      >;
    }
